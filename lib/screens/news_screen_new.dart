...
class _NewsScreenState extends State<NewsScreen> {
  ...
  String _cleanExcerpt(String? excerpt) {
    if (excerpt == null) return 'Sin descripción disponible';
    return excerpt
        .replaceAll(RegExp(r'<[^>]*>'), '')
        .replaceAll('&hellip;', '...')
        .replaceAll('&amp;', '&')
  ...
  String? _getFeaturedImage(Map<String, dynamic> post) {
    try {
      final embedded = post['_embedded'];
      if (embedded == null) return null;

      final featuredMedia = embedded['wp:featuredmedia'];
      if (featuredMedia == null || featuredMedia.isEmpty) return null;

      final mediaDetails = featuredMedia[0]['media_details'];
      if (mediaDetails == null) return null;

      final sizes = mediaDetails['sizes'];
      if (sizes == null) return null;

      // Intentar obtener tamaño mediano, si no está disponible usar el original
